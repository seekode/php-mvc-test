...
la 3 premières lignes ouvrent php, charge utils.php et charge splAutoload.php respectivement 

ensuite l'URL demandée est skockée dans $path, puis si cette URL est '/', on charge le controller de index

sinon, on explose l'URL à chaque '/' puis on met le deuxième élément de liste dans $path avant de stocké

un chemin vers un controllers avec cet élément puis si le fichier existe, il est chargé sinon on charge la view 404
...
cette fonction permet d'afficher un template ou un view avec les variables qui sont donné dans $data
...
1. models/User ligne 10 à 13 ligne 20 à 27

   créer un fonction pour modifier un valeur au lieu de le faire directement permet de forcer la validation des données de l'utilisateur, accentué par le fait que les variable soit privé

2. models/User ligne 39

   l'utilisation de htmlspecialchars() permet d'éviter une attaque XSS

3. models/User ligne 47

   l'utilisation de password_hash() permet d'éviter que même s'il y a une fuite dans la base de donnée les mot de passe sont sécurisé
